...
export class Shop {
  ...
  static async webCheckout(ctxt: HandlerContext, @ArgOptional key: string): Promise<string> {
    ...
    const handle = await ctxt.invoke(Shop, key).paymentWorkflow(PRODUCT_ID);
  ...
  static async paymentWorkflow(ctxt: WorkflowContext, productId: number): Promise<void> {
    ...
    // add a read only tx func call to test result buffer
    const product = await ctxt.invoke(ShopUtilities).retrieveProduct(productId);
    ctxt.logger.info(`Product: ${product.product} - Inventory: ${product.inventory}`);

    // Attempt to update the inventory. Signal the handler if it fails.
    try {
      await ctxt.invoke(ShopUtilities).subtractInventory(productId);
    } catch (error) {
      ctxt.logger.error("Failed to update inventory");
      await ctxt.setEvent(PAYMENT_URL_EVENT, null);
      return;
    }
    const orderID = await ctxt.invoke(ShopUtilities).createOrder(productId);
    ...
    if (notification && notification === 'paid') {
      ...
      await ctxt.invoke(ShopUtilities).undoSubtractInventory(productId);
    }
